...
export async function updateSecurityCredentials({ runtimeKey, currentCryptoMeta, currentPassword, newPassword, newPin }) {
  if (!runtimeKey) {
    throw new Error("Keine aktive Runtime-Session vorhanden.");
  }

  if (!currentCryptoMeta) {
    throw new Error("Keine Kryptometadaten vorhanden.");
  }

  const nextCryptoMeta = {
    ...currentCryptoMeta
  };

  if (newPassword) {
    const valid = await verifyPracticePassword({
      password: currentPassword || "",
      cryptoMeta: currentCryptoMeta
    });
    ...
    const passwordSaltBytes = generateSalt(16);
    nextCryptoMeta.passwordSaltBase64 = toBase64(passwordSaltBytes);
    nextCryptoMeta.wrappedDataKeyByPassword = await wrapDataKeyWithPassword(
      runtimeKey,
      newPassword,
      passwordSaltBytes
    );
  ...
  if (newPin) {
    const pinSaltBytes = generateSalt(16);
    nextCryptoMeta.pinSaltBase64 = toBase64(pinSaltBytes);
    nextCryptoMeta.wrappedDataKeyByPIN = await wrapDataKeyWithPIN(
      runtimeKey,
      newPin,
      pinSaltBytes
    );
  ...
  await saveCryptoMeta(nextCryptoMeta);
  return nextCryptoMeta;
}
